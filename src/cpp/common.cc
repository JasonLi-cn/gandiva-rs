...
namespace gandiva {
    ...
    public:
        RustResizableBuffer(RustMutableBuffer *mutable_buffer, RustMutableBuffers *mutable_buffers, rust_mutable_buffer_reserve_fn reserve_fn, uintptr_t idx)
                : arrow::ResizableBuffer(reinterpret_cast<uint8_t *>(mutable_buffer->addr), mutable_buffer->size),
    ...
    Status make_record_batch_with_buf_addrs(const SchemaPtr &schema, int64_t num_rows,
        ...
        for (int i = 0; i < num_fields; i++) {
            ...
            if (validity_size == 0) {
                ...
                auto validity = std::make_shared<arrow::Buffer>(reinterpret_cast<uint8_t *>(validity_addr),validity_size);
            ...
            if (arrow::is_binary_like(field->type()->id())) {
                ...
                auto offsets = std::make_shared<arrow::Buffer>(reinterpret_cast<uint8_t *>(offsets_addr), offsets_size);
            ...
            auto data = std::make_shared<arrow::Buffer>(reinterpret_cast<uint8_t *>(value_addr), value_size);
    ...
    ModuleId BuildProjector(ProtoMessage pb_schema,
        ...
        gandiva::Status status = Projector::Make(schema_ptr, expr_vector, mode, config, &projector);
        if (!status.ok()) {
            std::cout << "Failed to make LLVM module due to " << status.message() << "\n";
        }

        // store the result in a map
        std::shared_ptr<ProjectorHolder> holder = std::make_shared<ProjectorHolder>(schema_ptr, ret_types,
                                                                                    std::move(projector));
...
#define CHECK_OUT_BUFFER_IDX_AND_BREAK(idx, len)                               \
  if (idx >= len) {                                                            \
    ...
    void EvaluateProjector(ModuleId module_id,
        ...
        do {
            ...
            for (const FieldPtr &field: ret_types) {
                ...
                uintptr_t validity_addr = validity_buf.addr;
                int64_t validity_size = validity_buf.size;
                buffers.push_back(std::make_shared<arrow::MutableBuffer>(reinterpret_cast<uint8_t *>(validity_addr), validity_size));
                ...
                if (arrow::is_binary_like(field->type()->id())) {
                    ...
                    uintptr_t offsets_addr = offsets_buf.addr;
                    int64_t offsets_size = offsets_buf.size;
                    buffers.push_back(std::make_shared<arrow::MutableBuffer>(reinterpret_cast<uint8_t *>(offsets_addr), offsets_size));
                }
                ...
                if (arrow::is_binary_like(field->type()->id())) {
                    ...
                    buffers.push_back(std::make_shared<RustResizableBuffer>(&value_buf, mutable_buffers, reserve_fn, idx));
                } else {
                    uintptr_t data_addr = value_buf.addr;
                    int64_t data_size = value_buf.size;
                    buffers.push_back(std::make_shared<arrow::MutableBuffer>(reinterpret_cast<uint8_t *>(data_addr), data_size));
                }
    ...
    ModuleId BuildFilter(ProtoMessage pb_schema,
        ...
        // TODO releaseFilterInput(schema_arr, schema_bytes, condition_arr, condition_bytes, env);
        return module_id;
    }
    ...
    int32_t EvaluateFilter(ModuleId module_id,
        ...
        do {
            std::shared_ptr<arrow::RecordBatch> in_batch;
            status = make_record_batch_with_buf_addrs(holder->schema(), num_rows, in_bufs, in_bufs_len, &in_batch);
            if (!status.ok()) {
                break;
            }
            ...
            switch (selection_vector_type) {
                case gandiva::types::SV_INT16:
                    status = gandiva::SelectionVector::MakeInt16(num_rows, out_buffer, &selection_vector);
                    break;
                case gandiva::types::SV_INT32:
                    status = gandiva::SelectionVector::MakeInt32(num_rows, out_buffer, &selection_vector);
                    break;
                default:
