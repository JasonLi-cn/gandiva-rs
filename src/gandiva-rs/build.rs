...
fn find_llvm() -> Option<String> {
    let arrow_llvm_versions = "17.0;16.0;15.0;14.0;13.0;12.0;11.1;11.0;10;9;8;7";
    let versions: Vec<&str> = arrow_llvm_versions.split(';').collect();
...
fn find_llvm_linux() -> Option<String> {
        ...
        if let Ok(dirs) = fs::read_dir(&path) {
            for dir in dirs {
                    ...
                    .starts_with("llvm")
                {
                    ...
                    let p = if lib.is_dir() { lib } else { maybe_llvm_path };
