...
impl FieldNode {
    pub fn create(field: Field) -> Self {
        Self { field }
    }
}
...
impl TreeNode for FieldNode {
    fn to_protobuf(self) -> GandivaResult<crate::proto::TreeNode> {
        let field_node = crate::proto::FieldNode {
            field: Some(ArrowTypeHelper::arrow_field_to_protobuf(&self.field)?),
        };

        Ok(crate::proto::TreeNode {
            field_node: Some(field_node),
            ..Default::default()
        })
    }
}
